...
draft: false
featured: true
ogImage: "/posts/one-line-partitions/ducks-mid.png"
postSlug: one-line-partitions-with-duckdb
pubDatetime: 2023-05-24 8:00:00
tags: [data, tutorial]
title: From zero to data lake with DuckDB
emoji: 🐥
---
...
DuckDB utilizes a _vectorized_ query engine. This is a fancy way of saying it operates in a columnar fashion. If you're a data science ninja 🥷, you might be familiar with the [complexities](https://stackoverflow.com/questions/52673285/performance-of-pandas-apply-vs-np-vectorize-to-create-new-column-from-existing-c) of vectorized dataframe operations (they're a nightmare!)

Being a vectorized engine just means that large batches of values (vectors) are processed in a single operation. Vectorized execution leads to better performance in analytical calculations, which are often performed over _all_ values in a particular column, e.g. an aggregation (`SUM`, `MAX`) or window (`ROW_NUMBER`).

![A graphic describing the difference between row- & column-oriented databases.](/posts/one-line-partitions/row-vs-col.png)

While not a _new_ trend, there are many recent developments: [PyArrow](https://arrow.apache.org/docs/python/index.html), [Polars](https://github.com/pola-rs/polars), [Pandas 2.0](https://pandas.pydata.org/docs/dev/whatsnew/v2.0.0.html), _and_ [DuckDB](https://duckdb.org/) all tools built on in-memory, vectorized operations. Usually, when many things point in the same direction, it's wise to take note! 📝

Interestingly, the _features_ of DuckDB are not new— we've had OLAP DBMS for years: the real innovation behind DuckDB is its **simplicity** and **distribution** (free & [open-source](https://github.com/duckdb/duckdb)).

## 🫵 Why you should care

Most databases are _complicated_. While, today, we have managed solutions ([Amazon RDS](https://aws.amazon.com/rds/postgresql/), etc), they can be expensive, _especially_ managed OLAP solutions (as I'm sure many Snowflake customers can attest).

Not only is configuration a headache but truly _understanding_ how pricing works can be a nightmare. Thankfully, BigQuery has transparent pricing, but on the whole, cloud service pricing is opaque. This is a problem for production systems, but also for those seeking to _learn_ and _play_— it creates barriers to new data engineers.

Here's an example: say I want to install Postgres to learn about databases (as I'm sure many have done in their data journeys). Navigating to [Chapter 17](https://www.postgresql.org/docs/) (!) of the Postgres docs, we find _Installation from Source Code_, which sounds both intimidating and not fun. It's telling that section 17.1 is titled "Short Version," which already makes me nervous about what lies ahead. This will be a daunting quest. ⚔️

![Screenshot of the Postgres docs, Chapter 17— "Installation From Source Code."](/posts/one-line-partitions/psql-source.png)
...
So this is the part where I deliver some off-the-wall theories about how this will be awesome.

![My opinions on data tooling might make me a conspiracy theorist.](/posts/one-line-partitions/ancient-aliens.png)
...
Hear me out— imagine you have a data-lake situation. While SQL has its pitfalls, it is an excellent language for transforming data. With DuckDB, you could pull in semi-structured data, write it to parquet, then perform light SQL transformations in a data lake. Using a tool like dbt (or SQLMesh), you have a versioned, transformation-as-code system for transforming data.

Then, with a connector tool, you could write that transformed data to your final destination, whether that's _another_ DuckDB-DB or something like Postgres, Redshift, etc. This brings the benefit of databases and structured data _up a layer_, from analysts to data engineers. I could imagine DuckDB fitting into the final stages of [a medallion architecture](https://www.databricks.com/glossary/medallion-architecture)

Maybe that's a stretch, but I think it has potential. 🤔

![Dwayne "The Rock" Johnson KNOWS you have potential.](/posts/one-line-partitions/the-rock.gif)

### An _awesome_ opportunity for tutorials and learning

OK, so the earlier solutions are a bit meta, but this one is tangible and immediate. Most OLAP systems are hard to configure and difficult to learn. Most of the time, it takes _hours_ of fiddling just to be able to write some SQL. Furthermore, it's ambiguous _how much_ free cloud credits are worth, and I'm frequently worried that I'll be charged at some point for fiddling around in BigQuery. 😅

With DuckDB, we have a **free** way to set up training and sandbox environments. I think a SQL tutorial site built on fabricated data in DuckDB would be _awesome_.

As a former analyst, _the best_ way to learn SQL is by answering questions and playing around with a database. Though more resources exist currently (I remember leafing through [_Learning SQL_](https://www.oreilly.com/library/view/learning-sql-3rd/9781492057604/) back in the day), an interactive console built on DuckDB could be _invaluable_ for analytics training, and a staple in data engineering portfolio projects or proof-of-concepts.

Similarly, data engineers can benefit by integrating DuckDB into their end-to-end projects. Ben Rogojan has [an awesome post](https://www.theseattledataguy.com/7-data-engineering-projects-to-put-on-your-resume/#page-content) highlighting a few of these, but he forgot about DuckDB!

Why spend hours _configuring_ a source that you won't be able to maintain when you could simply drop DuckDB in a container?

![Billy Mays here: you must read on!](/posts/one-line-partitions/theres-more.png)
...
This is a pretty neat trick and something [Daniel Beech](https://www.confessionsofadataguy.com/duckdb-vs-polars-for-data-engineering/) first brought to light. Here, we'll take it one step further (that's right folks, you made it this far— you're getting the good stuff).

A common data-eng problem is taking some input (usually a CSV or JSON), partitioning it (chunking it up), and storing it in an appropriate format in cloud storage (parquet in s3).
...
I had trouble reading this URL directly using DuckDB, I suspect due to the Windows encoding (wtf FDIC 😑), so we'll use pandas. In the future, hopefully, DuckDB will support reading these CSVs directly.
...
I also cleaned up the columns a bit— there were some trailing spaces (the FDIC data processing team is crushing it).
...
![Screenshot of S3 bucket containing partitioned folder structure.](/posts/one-line-partitions/parts.png)

Maybe I'm just easily impressed, but I feel this is _really_ neat. You could take it one step further and write a class that makes this a bit more robust.
...
One of my goals for this blog is to generate excitement for promising data engineering tech. DuckDB is a _super_ accessible tool that has already generated a panoply of possibilities. I am exuberant about what will come of it, both from the open-source community \_and\_\_ DuckDB labs.

If you have other use cases for DuckDB that I missed (or if you think I'm wrong), please reach out! I'd love to hear your thoughts. My <a href="/about/">About</a> page has several ways to get in touch. 🙏
